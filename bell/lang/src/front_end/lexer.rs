    ...
    recursive(|meta_tokens| {
        let identifier = text::ident::<char, _>()
            .map(|identifier| match identifier.as_str() {
                ...
                _ => Token::Identifier(Intern::new(identifier)),
            })
            .labelled("identifier");
        ...
        let block = meta_tokens
            .clone()
            .delimited_by('{', '}')
            .recover_with(recovery::nested_delimiters('{', '}', [], |_| {
                Default::default()
            }))
            .map(MetaToken::Block);
...
pub fn lex(path: Intern<Utf8PathBuf>, text: &str) -> (Option<Vec<(MetaToken, Span)>>, Vec<Error>) {
