...
use crate::core::{error, util};
use crate::frontend::lexer::SpanToken;
...
impl<'a> TypeChecker<'a> {
    ...
    fn function<'b>(
        ...
        if let Some(Symbol {
            ...
            return Err(error::Error {
                ...
                error: error::ErrorKind::ShadowedSymbol {
                    ...
                    name: util::extract_id(function.id.clone()).0,
            ...
            }) = table.get_symbol_in_current(id)
            {
                return Err(error::Error {
                    ...
                    error: error::ErrorKind::ShadowedSymbol {
                        old: (**span).clone(),
                        new: parameter_span,
                        name: util::extract_id(id.clone()).0,
        ...
        function.return_type = Some(self.insert_or_check(function.return_type.clone(), tail)?);
    ...
    fn variable<'b>(
        ...
        }) = variable
        {
            ...
            let variable = util::extract_id(identifier.clone());
        ...
        match &declaration.hint {
            Some(return_type) => {
                if return_type != &inferred_type {
                    let hint = util::extract_id(declaration.hint.take().unwrap());
    ...
    fn assign<'b>(
        ...
        if let Some(Symbol {
            ...
            let variable = util::extract_id(assign.id.clone());
    ...
    fn while_loop<'b>(
        ...
        let condition_type = util::extract_id(self.expression(&mut while_loop.condition, table)?);
    ...
    fn conditional<'b>(
        ...
        for branch in conditional.branches.iter_mut() {
            let condition_type = util::extract_id(self.expression(&mut branch.condition, table)?);
            let boolean = String::from("bool");
            ...
            conditional_type = Some(self.insert_or_check(conditional_type, branch_type)?);
        }
    ...
    // Used for conditionals mostly
    fn insert_or_check(
        &self,
        mut expected: Option<SpanToken>,
        got: SpanToken,
    ) -> Result<SpanToken, error::Error<'a>> {
        match expected {
            Some(_) if expected.as_ref().unwrap() != &got => {
                let got = util::extract_id(got);
                let expected = util::extract_id(expected.unwrap());
        ...
        if let Some(Symbol {
            ...
            if call_parameters.len() != parameters.len() {
                return Err(error::Error {
                    ...
                    error: error::ErrorKind::ParameterCountMismatch {
                        ...
                        function: util::extract_id(call.id.clone()).0,
                    },
                });
            }
                ...
                .zip(parameters.iter())
            {
                if expected_parameter != &call_parameter {
                    let expected_parameter = util::extract_id((**expected_parameter).clone());
                    let call_parameter = util::extract_id((*call_parameter).clone());
            ...
            let id = util::extract_id(call.id.clone());
