...
impl<'a> Parser<'a> {
    ...
    fn constant(&mut self) -> Result<Constant, error::Error<'a>> {
        if !matches!(self.current, Some((lexer::Token::Constant, _))) {
            let (name, range) = self.token_report_data();
    ...
    fn expression(&mut self) -> Result<Expression, error::Error<'a>> {
        Ok(match self.current {
            ...
            Some((lexer::Token::Constant, _)) => Expression::Constant(Box::from(self.constant()?)),
